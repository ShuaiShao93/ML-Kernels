...
void CudnnDepthwiseConv3D(const float *A, const float *B, float *C, int N,
                          int D, int H, int W, int kD, int kH, int kW, int in_C,
  ...
  CHECK_CUDNN(cudnnSetConvolutionNdDescriptor(
      ...
      CUDNN_DATA_FLOAT));
  CHECK_CUDNN(cudnnSetConvolutionGroupCount(conv_desc, in_C));
...
int main(int argc, char **argv) {
  ...
  int N = 2, D = 10, H = 30, W = 20, in_C = 32, kD = 3, kH = 3, kW = 3;
  int size_A = N * D * H * W * in_C, size_B = in_C * kD * kH * kW * 1,
      size_C = N * D * H * W * in_C;
  float *A, *B, *C;
                ...
                for (int kh = 0; kh < kH; kh++) {
                  for (int kw = 0; kw < kW; kw++) {
                    ...
                    if (d - kD / 2 + kd < 0 || d - kD / 2 + kd >= D ||
                      ...
                      a = host_A[n * stride_an + (d - kD / 2 + kd) * stride_ad +
                                 (h - kH / 2 + kh) * stride_ah +
                                 (w - kW / 2 + kw) * stride_aw +
                                 ic * stride_aic];
                    }
